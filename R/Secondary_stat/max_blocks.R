...
atr_blocks <- read.table('./R/GRIMM/output_data/blocks/blocks.txt')[,c(1, 6:9)]
...
topmost_blocks <- data.frame(atr_blocks %>%
  group_by(V6) %>%
...
alb_ids_set <- apply(topmost_blocks, 1, function(row){
  ...
  start = as.numeric(row[3])
  end = start + as.numeric(row[4])
  
  alb_ids <- filter(atr_grimm, V7 >= start, V7 <= end)[, 1]
})

names(alb_ids_set) <- topmost_blocks[, 1]
...
block_coords[block_coords$ngenes > 3, ]
